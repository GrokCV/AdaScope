...
#*在这个模块内部，seg_feat 和 cls_feat 的信息流是独立的，
#它们之间没有直接的交叉。但是，因为它们共享了 LSKModule 的所有卷积层权重，
#所以模块学习到的是一个通用的特征增强策略，这个策略同时对分割任务和分类任务的特征都有益。

#*总结来说，LSKModule 在这里充当了一个强大的“特征处理器”，
#它不是简单地用固定大小的卷积核去处理特征，而是让网络根据任务和图像内容，
#智能地为每个像素位置“调配”最合适的感受野，从而输出包含了更丰富、更有效信息的特征图，以供后续的网络层使用*# 
