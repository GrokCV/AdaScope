...
model = dict(
    ...
    bbox_head=dict(
        ...
        reg_max=4,
        loss_cls=dict(type="QualityFocalLoss", use_sigmoid=True, beta=4.0, loss_weight=0.1),
        loss_dfl=dict(type="DistributionFocalLoss", loss_weight=0.25),
        loss_bbox=dict(type="GIoULoss", loss_weight=2.0),
    ),
    train_cfg=dict(assigner=dict(type="ATSSAssigner", topk=3), allowed_border=-1, pos_weight=-1, debug=False),
    test_cfg=dict(nms_pre=3000, min_bbox_size=0, score_thr=0.001, nms=dict(type="nms", iou_threshold=0.6), max_per_img=300),
)
