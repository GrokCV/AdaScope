...
class RefinerStageSwitchHook(Hook):
    """Enable refiner after a given epoch and disable it before then."""

    def __init__(self, enable_epoch: int = 12, enabled_alpha: float = 1.0, disabled_alpha: float = 0.0):
    ...
    def _apply(self, runner: Runner, mode: str) -> None:
        ...
        runner.logger.info(
            f'[RefinerStageSwitchHook] mode={mode} epoch={runner.epoch} enable_refiner={enable_refiner}'
        )
