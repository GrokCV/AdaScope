...
class LoadAnnotationsCluster(MMCV_LoadAnnotations):
    ...
    def _load_clusters(self, results: dict) -> None:
        """Private function to load bounding box annotations.
        ...
        gt_bboxes = []
        gt_ignore_flags = []
        for instance in results.get('cluster_instances', []):
            gt_bboxes.append(instance['bbox'])
            gt_ignore_flags.append(instance['ignore_flag'])
        if self.box_type is None:
            results['cluster_bboxes'] = np.array(
                gt_bboxes, dtype=np.float32).reshape((-1, 4))
        else:
            _, box_type_cls = get_box_type(self.box_type)
            results['cluster_bboxes'] = box_type_cls(gt_bboxes, dtype=torch.float32)
        results['cluster_ignore_flags'] = np.array(gt_ignore_flags, dtype=bool)    
