...
class CopyPaste(BaseTransform):
    ...
    def generate_gaussian_matrix(self, h, w, sigma_x=0.8, sigma_y=0.6, mu_x=0.0, mu_y=0.0, theta=0.0):
        ...
        gaussian_max = gaussian.max()
        if gaussian_max <= 0:
            return gaussian
        gaussian /= gaussian_max
        return gaussian
...
class SkyCopyPaste(BaseTransform):
    ...
    def _copy_paste(self, dst_results: dict, src_results: dict) -> dict:
        ...
        # Resolve mask paths from the current sample image path instead of a fixed dataset root.
        img_path = dst_results['img_path']
        img_basename = osp.basename(img_path)
        dataset_root = osp.dirname(osp.dirname(img_path))
        dst_sky_path = osp.join(
            dataset_root, 'SkySeg', 'BinaryMask',
            img_basename.replace('.png', '_pixels0.png'))
        ...
        # 读取原图像mask
        dst_mask_path = osp.join(
            dataset_root, 'SIRST', 'BinaryMask',
            img_basename.replace('.png', '_pixels0.png'))
