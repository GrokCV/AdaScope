...
class SIRSTVOCDetDataset(XMLDataset):
    ...
    def __init__(self, 
                 ...
                 ann_tag='object',  # <---- [新] 增加一个参数，用于指定要解析的XML标签
                 **kwargs):
        self.ann_tag = ann_tag  # <---- [新] 将参数保存为类的属性
        print(f"INFO: This dataset will load annotations from <{self.ann_tag}> tags.")
    ...
    def _parse_instance_info(
        ...
        """
        instances = []
        # v---- [修改] 不再写死 "object"，而是使用 self.ann_tag 参数
        for obj in raw_ann_info.findall(self.ann_tag):
            instance = {}
            name = obj.find("name").text
            if name not in self._metainfo["classes"]:
                continue
            
            # 注意: <cluster> 标签可能没有 <difficult> 标签，需要做兼容处理
